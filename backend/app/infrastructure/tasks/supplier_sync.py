...
# Quote → price/stock row mapping lives in a shared module so the component
# ingestion service can reuse the exact same upsert for a new component's
# first snapshot. Imported under the historical private names to keep the
# call sites (and the unit test) below unchanged.
from app.infrastructure.supplier_snapshot import (
    ensure_supplier_row as _ensure_supplier_row,
)
from app.infrastructure.supplier_snapshot import (
    upsert_prices_and_stock as _upsert_prices_and_stock,
)
