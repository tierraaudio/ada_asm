...
# No result backend: nobody reads Celery results — task outcome lives in
# the `supplier_sync_runs` table. Dropping it removes a whole Redis
# dependency from the worker and the API producer.
celery_app = Celery(
    "ada_asm",
