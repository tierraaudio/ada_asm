...
describe("<DataTablePagination>", () => {
  it("renders the 'page X of Y · N resultados' summary by default", () => {
    render(
      <DataTablePagination page={2} pageCount={5} total={120} onPageChange={vi.fn()} />,
    );
  ...
  it("invokes onPageChange with the right targets for each arrow", async () => {
    ...
    await userEvent.click(
      screen.getByRole("button", { name: /Ir a la primera página/i }),
    );
    ...
    await userEvent.click(
      screen.getByRole("button", { name: /Ir a la última página/i }),
    );
  ...
  it("disables the left pair on the first page", () => {
    render(
      <DataTablePagination page={1} pageCount={5} total={120} onPageChange={vi.fn()} />,
    );
  ...
  it("disables the right pair on the last page", () => {
    render(
      <DataTablePagination page={5} pageCount={5} total={120} onPageChange={vi.fn()} />,
    );
