...
- [ ] 1.1 Add `Module` (frozen dataclass) at `backend/app/domain/entities/module.py` with fields per `specs/data-model.md`.
- [ ] 1.2 Add `ModuleChild` (frozen dataclass) at `backend/app/domain/entities/module_child.py`.
- [ ] 1.3 Add `ModuleRepository` Protocol at `backend/app/domain/repositories/module_repository.py` with method stubs: `list`, `get`, `get_with_aggregates`, `get_tree`, `create`, `update`, `delete`, `add_child`, `update_child`, `remove_child`, `list_price_history`, `_check_cycle`.
- [ ] 1.4 Add `ModuleNotFoundError`, `ModuleSkuAlreadyRegisteredError`, `ModuleCycleDetectedError`, `ChildAlreadyPresentError`, `InvalidChildReferenceError` to `backend/app/core/exceptions.py` with stable codes.
- [ ] 1.5 Create `backend/app/infrastructure/db/models/module.py` and `module_child.py` with SQLAlchemy mappings + CHECK + UNIQUE + indexes per `specs/data-model.md`.
- [ ] 1.6 Register both models in `backend/app/infrastructure/db/models/__init__.py`.
- [ ] 1.7 Generate Alembic migration `module_management__modules_and_children`. Manually edit to replace autogenerated `op.create_index` for the functional `lower(sku)` index with `op.execute(...)`. Validate upgrade+downgrade+upgrade.
