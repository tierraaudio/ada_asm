# supplier-sync-cron Specification

## Purpose
Daily Celery Beat job that iterates the catalogue per enabled supplier, upserts append-only `supplier_prices` + `supplier_stocks` history rows, captures per-run telemetry in `supplier_sync_runs`, per-component errors in `supplier_sync_errors`, and exposes admin endpoints for inspection + ad-hoc triggers.
