...
## Purpose
Adapter pattern + shared infrastructure for fetching electronic-component data from the five fixed distributors (Mouser, DigiKey, TME, Farnell, RS). Owns the `SupplierAdapter` Protocol, the `SupplierQuote` value object, the per-supplier MPN field mapping, the Redis token-bucket rate limiter, and the credential gate.
